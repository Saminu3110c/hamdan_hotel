...
if(isset($add))
{
	$sql=mysqli_query($conn,"select * from rooms_tbl where room_no='$rno'");
	...
	{	
	$img=$_FILES['img']['name'];
	mysqli_query($conn,"insert into rooms_tbl values('','$rno','$type','$price','$details','$img')");	
	move_uploaded_file($_FILES['img']['tmp_name'],"../images/rooms/".$_FILES['img']['name']);
